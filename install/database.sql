...
DELETE FROM `instagram_history`;

ALTER TABLE `settings` ADD `clear_log_days` VARCHAR(255) NULL DEFAULT NULL AFTER `timezone`;

ALTER TABLE `instagram_accounts` ADD `logs_counter` TEXT NULL DEFAULT NULL AFTER `status`;

ALTER TABLE `instagram_accounts` ADD `followed_username` LONGTEXT NULL DEFAULT NULL AFTER `logs_counter`;

UPDATE `settings` SET `clear_log_days` = '3' WHERE `settings`.`id` = 1;